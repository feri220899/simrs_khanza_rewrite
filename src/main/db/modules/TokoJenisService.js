...
// generik — tabelnya `tokojenisbarang(kd_jenis, nm_jenis)`.
...
async function list({ page = 1, pageSize = 10, sortOrder = 'asc', search = '' } = {}) {
    ...
    const { rows } = await db.query(
        `SELECT kd_jenis, nm_jenis FROM tokojenisbarang
         WHERE kd_jenis ILIKE $1 OR nm_jenis ILIKE $1
         ORDER BY kd_jenis ${dir}
         LIMIT $2 OFFSET $3`,
        [like, pageSize, (page - 1) * pageSize]
    )
    const { rows: [{ count }] } = await db.query(
        'SELECT count(*)::int AS count FROM tokojenisbarang WHERE kd_jenis ILIKE $1 OR nm_jenis ILIKE $1',
        [like]
    )
...
async function nextKode() {
    const db = await DatabaseService.get()
    const { rows } = await db.query('SELECT count(*)::int AS n FROM tokojenisbarang')
...
async function create(data) {
    ...
    const db = await DatabaseService.get()
    try {
        await db.query('INSERT INTO tokojenisbarang (kd_jenis, nm_jenis) VALUES ($1,$2)', [data.kd_jenis, data.nm_jenis])
        return { success: true }
    } catch (e) {
        if (e.code === '23505') return { success: false, message: `Kode "${data.kd_jenis}" sudah dipakai` }
        throw e
    }
}
...
async function update(oldKode, data) {
    ...
    try {
        const { rowCount } = await db.query(
            'UPDATE tokojenisbarang SET kd_jenis=$1, nm_jenis=$2 WHERE kd_jenis=$3',
...
async function deleteOne(kode) {
    const db = await DatabaseService.get()
    try {
        const { rowCount } = await db.query('DELETE FROM tokojenisbarang WHERE kd_jenis=$1', [kode])
