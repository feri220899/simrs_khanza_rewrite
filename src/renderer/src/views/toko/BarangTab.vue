...
// src/toko/TokoBarang.java — harga jual (distributor/grosir/retail) DIHITUNG
// OTOMATIS dari "Beli" x persentase toko_setharga, tapi tetap bisa ditimpa
// manual (auto-fill, bukan lock) — sama seperti Java asli. Hapus = SOFT
// DELETE, ada tab "Data Sampah" (restore) yang di aslinya dibatasi role
// "Admin Utama" — di sini digate role Administrator persis, bukan cuma
// permission toko_barang biasa (lihat main/index.js).
const { showToast } = useToast()
const authStore = useAuthStore()
const bolehTulis = () => authStore.can('toko_barang')
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.toko.barang.list(params),
    pageSize: 10,
    defaultSortBy: 'kode_brng',
})

const emptyForm = () => ({ kode_brng: '', nama_brng: '', kode_sat: '', jenis: '', dasar: '', h_beli: '', distributor: '', grosir: '', retail: '' })
const saving = ref(false)
const form = reactive(emptyForm())

async function siapkanFormBaru() {
    const next = await window.api.toko.barang.nextKode()
    Object.assign(form, emptyForm())
    form.kode_brng = next
}

async function hitungHarga() {
    if (!form.h_beli) return
    const h = await window.api.toko.barang.calcHarga(form.h_beli)
    form.distributor = h.distributor
    form.grosir = h.grosir
    form.retail = h.retail
}

async function simpan() {
    saving.value = true
    try {
        const res = await window.api.toko.barang.create(authStore.token, { ...form })
...
function openEdit(row) {
    ...
    Object.assign(editForm, {
        kode_brng: row.kode_brng, nama_brng: row.nama_brng, kode_sat: row.kode_sat, jenis: row.jenis,
        dasar: row.dasar, h_beli: row.h_beli, distributor: row.distributor, grosir: row.grosir, retail: row.retail,
    })
...
async function update() {
    updating.value = true
    try {
        const res = await window.api.toko.barang.update(authStore.token, editOldKode.value, { ...editForm })
        if (!res.success) { showToast(res.message, 'error'); return }
...
async function hapus(row) {
    ...
    const res = await window.api.toko.barang.delete(authStore.token, row.kode_brng)
    if (!res.success) { showToast(res.message, 'error'); return }
...
async function muatSampah() {
    ...
    try {
        const res = await window.api.toko.barang.listSampah(authStore.token, { pageSize: 100 })
...
async function restore(row) {
    ...
    const res = await window.api.toko.barang.restore(authStore.token, row.kode_brng)
    if (!res.success) { showToast(res.message, 'error'); return }
