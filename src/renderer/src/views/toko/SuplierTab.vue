...
// src/toko/TokoSuplier.java — 7 field, tidak ada kondisional -> pola SEDERHANA.
const { showToast } = useToast()
const authStore = useAuthStore()
const bolehTulis = () => authStore.can('toko_suplier')
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.toko.suplier.list(params),
    pageSize: 10,
...
async function siapkanFormBaru() {
    const next = await window.api.toko.suplier.nextKode()
    Object.assign(form, emptyForm())
    form.kode_suplier = next
}

async function simpan() {
    saving.value = true
    try {
        const res = await window.api.toko.suplier.create(authStore.token, { ...form })
...
async function update() {
    updating.value = true
    try {
        const res = await window.api.toko.suplier.update(authStore.token, editOldKode.value, { ...editForm })
        if (!res.success) { showToast(res.message, 'error'); return }
...
async function hapus(row) {
    ...
    const res = await window.api.toko.suplier.delete(authStore.token, row.kode_suplier)
    if (!res.success) { showToast(res.message, 'error'); return }
