...
// src/toko/TokoJenis.java — taksonomi jenis barang, 2 kolom.
const { showToast } = useToast()
const authStore = useAuthStore()
const bolehTulis = () => authStore.can('toko_jenis')
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.toko.jenis.list(params),
    pageSize: 10,
...
async function siapkanFormBaru() {
    form.kd_jenis = await window.api.toko.jenis.nextKode()
    form.nm_jenis = ''
}
...
async function simpan() {
    saving.value = true
    try {
        const res = await window.api.toko.jenis.create(authStore.token, { ...form })
...
async function update() {
    updating.value = true
    try {
        const res = await window.api.toko.jenis.update(authStore.token, editOldKode.value, { ...editForm })
        if (!res.success) { showToast(res.message, 'error'); return }
...
async function hapus(row) {
    ...
    const res = await window.api.toko.jenis.delete(authStore.token, row.kd_jenis)
    if (!res.success) { showToast(res.message, 'error'); return }
