...
import { Plus, List, Tags } from 'lucide-vue-next'
import { useServerTable } from '../../composables/useServerTable.js'
import { useToast } from '../../composables/useToast.js'
import { useAuthStore } from '../../stores/auth.js'
import AppPagination from '../../components/AppPagination.vue'

// src/ipsrs/IPSRSJenis.java — taksonomi jenis barang IPSRS, 2 kolom. Pola
// identik src/renderer/src/views/toko/JenisTab.vue (tokojenisbarang), cuma
// tabel & permission slug beda (`ipsrs_jenis_barang`).
const { showToast } = useToast()
const authStore = useAuthStore()
const bolehTulis = () => authStore.can('ipsrs_jenis_barang')
...
const columns = [
    ...
    { accessorKey: 'kd_jenis', header: 'Kode', meta: { headerClass: 'w-32', cellClass: 'font-medium' } },
    { accessorKey: 'nm_jenis', header: 'Jenis Barang' },
    {
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.ipsrs.jenis.list(params),
    pageSize: 10,
    defaultSortBy: 'kd_jenis',
})

const saving = ref(false)
const form = reactive({ kd_jenis: '', nm_jenis: '' })

async function siapkanFormBaru() {
    form.kd_jenis = await window.api.ipsrs.jenis.nextKode()
    form.nm_jenis = ''
}
...
async function simpan() {
    saving.value = true
    try {
        const res = await window.api.ipsrs.jenis.create(authStore.token, { ...form })
        if (!res.success) { showToast(res.message, 'error'); return }
        showToast('Jenis barang berhasil disimpan.')
...
const editForm = reactive({ kd_jenis: '', nm_jenis: '' })

function openEdit(row) {
    editOldKode.value = row.kd_jenis
...
async function update() {
    updating.value = true
    try {
        const res = await window.api.ipsrs.jenis.update(authStore.token, editOldKode.value, { ...editForm })
        if (!res.success) { showToast(res.message, 'error'); return }
        showToast('Jenis barang berhasil diupdate.')
...
async function hapus(row) {
    if (!confirm(`Hapus jenis "${row.nm_jenis}"?`)) return
    const res = await window.api.ipsrs.jenis.delete(authStore.token, row.kd_jenis)
    if (!res.success) { showToast(res.message, 'error'); return }
    showToast('Jenis barang berhasil dihapus.')
    fetchData()
}

function cetakDaftar() {
    const rows = table.getRowModel().rows.map((row, i) => `<tr><td>${i + 1}</td><td>${row.original.kd_jenis}</td><td>${row.original.nm_jenis}</td></tr>`).join('')
    const w = window.open('', '_blank', 'width=900,height=700')
    if (!w) { showToast('Popup cetak diblokir browser', 'error'); return }
    w.document.write(`<html><head><title>Master Jenis IPSRS</title><style>body{font:12px Arial;margin:24px}h1{text-align:center;font-size:18px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #555;padding:6px}th{background:#eee}</style></head><body><h1>MASTER JENIS BARANG IPSRS</h1><p>Filter: ${search || '-'}</p><table><thead><tr><th>No</th><th>Kode</th><th>Jenis Barang</th></tr></thead><tbody>${rows}</tbody></table></body></html>`)
...
<template>
        ...
        <div v-show="activeTab === 'list'" class="flex-1 min-h-0 overflow-hidden">
            <div class="bg-base-100 rounded-2xl border border-base-200 shadow-sm h-full flex flex-col overflow-hidden px-4 py-3">
                <div class="flex justify-end mb-2">
                    <button class="btn btn-ghost btn-sm text-primary" @click="cetakDaftar">Cetak Daftar</button>
                </div>
                                ...
                                <td colspan="4" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <div class="size-14 rounded-2xl bg-base-200 flex items-center justify-center">
                                            <Tags class="size-7 text-base-content/30" />
                                        </div>
                                        <p class="font-semibold text-base-content/60">Belum ada jenis barang</p>
                                    </div>
                                </td>
                            </tr>
        ...
        <div v-show="activeTab === 'tambah'" class="flex-1 overflow-y-auto pb-6">
            <div class="bg-base-100 rounded-2xl border border-base-200 shadow-sm overflow-hidden">
                <div class="px-5 py-3.5 border-b border-base-200 flex items-center gap-3">
                    ...
                    <h3 class="font-semibold text-base-content">Informasi Jenis Barang</h3>
                </div>
                <div class="p-5">
    ...
    <dialog ref="modalEdit" class="modal">
        <div class="modal-box max-w-sm">
            <h3 class="font-bold text-base mb-4">Edit Jenis Barang</h3>
            <div class="space-y-3">
                <div>
                    <label class="block text-sm font-medium text-base-content/80 mb-1.5">Kode</label>
