...
async function muatOpsi() {
    const [a, i, d] = await Promise.all([
        ...
        window.api.perpustakaan.denda.list({ pageSize: 1000 }),
    ])
    opsiAnggota.value = a.data
    opsiInventaris.value = i.data
    opsiJenisDenda.value = d.data
}
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.perpustakaan.bayarDenda.listLain(params),
    pageSize: 10,
    defaultSortBy: 'tgl_denda',
    defaultSortOrder: 'desc',
})

const emptyForm = () => ({ tgl_denda: new Date().toISOString().slice(0, 10), no_anggota: '', no_inventaris: '', kode_denda: '', keterangan_denda: '' })
...
async function simpan() {
    saving.value = true
    try {
        const res = await window.api.perpustakaan.bayarDenda.createLain(authStore.token, { ...form })
        if (!res.success) { showToast(res.message, 'error'); return }
        showToast(`Denda berhasil dicatat (Rp ${Number(res.besarDenda).toLocaleString('id-ID')}).`)
...
async function hapus(row) {
    ...
    const res = await window.api.perpustakaan.bayarDenda.deleteLain(authStore.token, {
        tgl_denda: row.tgl_denda, no_anggota: row.no_anggota, no_inventaris: row.no_inventaris, kode_denda: row.kode_denda,
    })
    if (!res.success) { showToast(res.message, 'error'); return }
                ...
                <AppPagination :table="table" v-model:search="search" class="flex-1 min-h-0">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr v-else-if="table.getRowModel().rows.length === 0"><td colspan="6" class="py-16 text-center text-base-content/50">Belum ada catatan denda lain-lain</td></tr>
        ...
        <div v-show="activeTab === 'tambah'" class="flex-1 overflow-y-auto pb-6">
            <div class="bg-base-100 rounded-2xl border border-base-200 shadow-sm overflow-hidden">
                <div class="px-5 py-3.5 border-b border-base-200 flex items-center gap-3">
                    ...
                    <h3 class="font-semibold text-base-content">Catat Denda Lain-lain</h3>
                </div>
                <div class="p-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-xl">
                        <div>
                            <label class="block text-sm font-medium text-base-content/80 mb-1.5">Tanggal</label>
                            <input v-model="form.tgl_denda" type="date" class="input input-bordered input-sm w-full" />
                        </div>
                        <div></div>
                        <div>
                        ...
                        <div>
                            ...
                            <input v-model="form.keterangan_denda" type="text" class="input input-bordered input-sm w-full" @keyup.enter="simpan" />
                        </div>
                    </div>
