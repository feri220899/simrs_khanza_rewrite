...
// src/perpustakaan/PerpustakaanPenerbit.java — 6 kolom (bukan taksonomi 2
// kolom biasa), tapi field lain di luar kode/nama TIDAK saling bergantung
// (tidak ada kondisional) jadi tetap pola SEDERHANA (2 tab + modal edit)
// sesuai Konvensi UI, bukan modal tunggal ala entity kompleks.
const { showToast } = useToast()
const authStore = useAuthStore()
const bolehTulis = () => authStore.can('penerbit_perpustakaan')
...
const columns = [
    ...
    { accessorKey: 'kode_penerbit', header: 'Kode', meta: { headerClass: 'w-28', cellClass: 'font-medium' } },
    { accessorKey: 'nama_penerbit', header: 'Nama Penerbit' },
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.perpustakaan.penerbit.list(params),
    pageSize: 10,
    defaultSortBy: 'kode_penerbit',
})

const emptyForm = () => ({ kode_penerbit: '', nama_penerbit: '', alamat_penerbit: '', no_telp: '', email: '', website_penerbit: '' })

const saving = ref(false)
const form = reactive(emptyForm())

async function siapkanFormBaru() {
    const next = await window.api.perpustakaan.penerbit.nextKode()
    Object.assign(form, emptyForm())
    form.kode_penerbit = next
}

async function simpan() {
    saving.value = true
    try {
        const res = await window.api.perpustakaan.penerbit.create(authStore.token, { ...form })
        if (!res.success) { showToast(res.message, 'error'); return }
        showToast('Penerbit berhasil disimpan.')
...
function openEdit(row) {
    editOldKode.value = row.kode_penerbit
...
async function update() {
    updating.value = true
    try {
        const res = await window.api.perpustakaan.penerbit.update(authStore.token, editOldKode.value, { ...editForm })
        if (!res.success) { showToast(res.message, 'error'); return }
        showToast('Penerbit berhasil diupdate.')
...
async function hapus(row) {
    if (!confirm(`Hapus penerbit "${row.nama_penerbit}"?`)) return
    const res = await window.api.perpustakaan.penerbit.delete(authStore.token, row.kode_penerbit)
    if (!res.success) { showToast(res.message, 'error'); return }
    showToast('Penerbit berhasil dihapus.')
    fetchData()
}
...
<template>
        ...
        <div v-show="activeTab === 'tambah'" class="flex-1 overflow-y-auto pb-6">
            <div class="bg-base-100 rounded-2xl border border-base-200 shadow-sm overflow-hidden">
                <div class="px-5 py-3.5 border-b border-base-200 flex items-center gap-3">
                    ...
                    <h3 class="font-semibold text-base-content">Informasi Penerbit</h3>
                </div>
                <div class="p-5">
    ...
    <dialog ref="modalEdit" class="modal">
        <div class="modal-box max-w-md">
            <h3 class="font-bold text-base mb-4">Edit Penerbit</h3>
            <div class="space-y-3">
                <div>
                    <label class="block text-sm font-medium text-base-content/80 mb-1.5">Kode</label>
