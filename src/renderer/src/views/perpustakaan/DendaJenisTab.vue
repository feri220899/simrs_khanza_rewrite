...
import { Plus, List, Coins } from 'lucide-vue-next'
import { useServerTable } from '../../composables/useServerTable.js'
import { useToast } from '../../composables/useToast.js'
import { useAuthStore } from '../../stores/auth.js'
import AppPagination from '../../components/AppPagination.vue'

// src/perpustakaan/PerpustakaanDenda.java — taksonomi jenis denda "lain-lain",
// `besar_denda` = PERSENTASE dari harga buku (bukan nominal tetap).
const { showToast } = useToast()
const authStore = useAuthStore()
const bolehTulis = () => authStore.can('denda_perpustakaan')
...
const columns = [
    ...
    { accessorKey: 'kode_denda', header: 'Kode', meta: { headerClass: 'w-28', cellClass: 'font-medium' } },
    { accessorKey: 'jenis_denda', header: 'Jenis Denda' },
    {
        accessorKey: 'besar_denda', header: '% dari Harga Buku', meta: { headerClass: 'w-40 text-right', cellClass: 'text-right tabular-nums' },
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.perpustakaan.denda.list(params),
    pageSize: 10,
    defaultSortBy: 'kode_denda',
})

const saving = ref(false)
const form = reactive({ kode_denda: '', jenis_denda: '', besar_denda: '' })

async function siapkanFormBaru() {
    form.kode_denda = await window.api.perpustakaan.denda.nextKode()
    form.jenis_denda = ''
    form.besar_denda = ''
}
...
async function simpan() {
    saving.value = true
    try {
        const res = await window.api.perpustakaan.denda.create(authStore.token, { ...form })
        if (!res.success) { showToast(res.message, 'error'); return }
        showToast('Jenis denda berhasil disimpan.')
...
const editForm = reactive({ kode_denda: '', jenis_denda: '', besar_denda: '' })

function openEdit(row) {
    editOldKode.value = row.kode_denda
...
async function update() {
    updating.value = true
    try {
        const res = await window.api.perpustakaan.denda.update(authStore.token, editOldKode.value, { ...editForm })
        if (!res.success) { showToast(res.message, 'error'); return }
        showToast('Jenis denda berhasil diupdate.')
...
async function hapus(row) {
    if (!confirm(`Hapus jenis denda "${row.jenis_denda}"?`)) return
    const res = await window.api.perpustakaan.denda.delete(authStore.token, row.kode_denda)
    if (!res.success) { showToast(res.message, 'error'); return }
    showToast('Jenis denda berhasil dihapus.')
    fetchData()
}
...
<template>
                                ...
                                <td colspan="4" class="py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        <div class="size-14 rounded-2xl bg-base-200 flex items-center justify-center">
                                            <Coins class="size-7 text-base-content/30" />
                                        </div>
                                        <p class="font-semibold text-base-content/60">Belum ada jenis denda</p>
                                    </div>
                                </td>
                            </tr>
        ...
        <div v-show="activeTab === 'tambah'" class="flex-1 overflow-y-auto pb-6">
            <div class="bg-base-100 rounded-2xl border border-base-200 shadow-sm overflow-hidden">
                <div class="px-5 py-3.5 border-b border-base-200 flex items-center gap-3">
                    ...
                    <h3 class="font-semibold text-base-content">Informasi Jenis Denda</h3>
                </div>
                <div class="p-5">
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-xl">
                        <div>
                            ...
                            <input v-model="form.kode_denda" type="text" maxlength="20" class="input input-bordered input-sm w-full" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-base-content/80 mb-1.5">Jenis Denda <span class="text-error">*</span></label>
                            <input v-model="form.jenis_denda" type="text" maxlength="150" class="input input-bordered input-sm w-full" placeholder="Contoh: Buku Rusak" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-base-content/80 mb-1.5">% dari Harga Buku <span class="text-error">*</span></label>
                            <input v-model="form.besar_denda" type="number" min="0" max="100" class="input input-bordered input-sm w-full" @keyup.enter="simpan" />
                        </div>
                    </div>
    ...
    <dialog ref="modalEdit" class="modal">
        <div class="modal-box max-w-sm">
            <h3 class="font-bold text-base mb-4">Edit Jenis Denda</h3>
            <div class="space-y-3">
                <div>
                    <label class="block text-sm font-medium text-base-content/80 mb-1.5">Kode</label>
                    <input v-model="editForm.kode_denda" type="text" maxlength="20" class="input input-bordered input-sm w-full" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-base-content/80 mb-1.5">Jenis Denda</label>
                    <input v-model="editForm.jenis_denda" type="text" maxlength="150" class="input input-bordered input-sm w-full" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-base-content/80 mb-1.5">% dari Harga Buku</label>
                    <input v-model="editForm.besar_denda" type="number" min="0" max="100" class="input input-bordered input-sm w-full" />
                </div>
            </div>
