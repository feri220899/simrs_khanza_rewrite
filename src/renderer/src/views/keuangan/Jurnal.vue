...
<template>
    <div class="flex flex-col h-full min-h-0">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 shrink-0">
            <div>
                <h1 class="text-xl font-bold flex items-center gap-2">
                    <BookText class="size-6 text-primary" /> Jurnal / Posting Jurnal
                </h1>
                <p class="text-sm text-base-content/60">Pengelolaan transaksi jurnal akuntansi</p>
            </div>
            <div class="flex items-center gap-2">
                <button class="btn btn-ghost btn-sm gap-2" @click="printList" :disabled="loading || !rows.length"><Printer class="size-4" /> Cetak Daftar</button>
                <button class="btn btn-primary btn-sm gap-2" @click="openNew"><Plus class="size-4" /> Jurnal Baru</button>
            </div>
        </div>
            ...
            <div class="card-body p-4 flex flex-row flex-wrap items-end gap-4">
                ...
                <div class="form-control w-full max-w-xs">
                    ...
                    <div class="join w-full">
                        <input type="text" v-model="search" placeholder="No Jurnal, No Bukti, Keterangan..." class="input input-bordered input-sm join-item w-full" @keyup.enter="load" />
        ...
        <AppPagination v-model:page="page" v-model:page-size="pageSize" :total="rows.length" :page-sizes="[10, 20, 50, 100]" :hide-search="true">
            <div class="overflow-x-auto border border-base-200 rounded-lg relative min-h-[300px] max-h-[60vh] overflow-y-auto">
                ...
                <table class="table table-sm w-full table-fixed">
                        ...
                        <template v-for="row in paginatedRows" :key="row.no_jurnal">
                            <tr class="bg-base-200/50">
                                ...
                                <td class="text-center">
                                    <button class="btn btn-ghost btn-xs text-primary" @click="printJurnal(row)" title="Cetak Jurnal">
                                        <Printer class="size-4" />
                                    </button>
                                    <button class="btn btn-ghost btn-xs text-error" @click="remove(row)" title="Hapus Jurnal">
                                        <Trash2 class="size-4" />
                                    </button>
                                </td>
                            </tr>
                    ...
                    <tbody v-else>
                        <tr>
                            <td colspan="7" class="text-center py-8 text-base-content/50">Tidak ada data jurnal.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <dialog class="modal" :class="{'modal-open': showModal}">
            <div class="modal-box p-0 rounded-2xl border border-base-200 w-11/12 max-w-5xl flex flex-col max-h-[90vh] overflow-hidden">
                <div class="bg-base-200/60 px-6 py-4 flex items-center justify-between border-b border-base-200 shrink-0">
                ...
                <div class="overflow-y-auto p-6 grow">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div class="flex flex-col gap-1.5">
                            <label class="block text-sm font-medium px-1">No. Jurnal <span class="text-error">*</span></label>
                            <input type="text" v-model="nextNoJurnal" class="input input-bordered input-sm w-full bg-base-200" readonly />
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="block text-sm font-medium px-1">No. Bukti</label>
                            <input type="text" v-model="form.no_bukti" class="input input-bordered input-sm w-full uppercase" maxlength="30" />
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="flex flex-col gap-1.5">
                                <label class="block text-sm font-medium px-1">Tanggal <span class="text-error">*</span></label>
                                <input type="date" v-model="form.tgl_jurnal" class="input input-bordered input-sm w-full" />
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="block text-sm font-medium px-1">Jam <span class="text-error">*</span></label>
                                <input type="time" v-model="form.jam_jurnal" step="1" class="input input-bordered input-sm w-full" />
                            </div>
                        </div>
                        <div class="flex flex-col gap-1.5">
                        ...
                        <div class="flex flex-col gap-1.5 md:col-span-2">
                            ...
                            <input type="text" v-model="form.keterangan" class="input input-bordered input-sm w-full uppercase" maxlength="350" />
                        </div>
                    </div>
                    ...
                    <div class="bg-base-200/50 p-4 rounded-xl border border-base-300 mb-4">
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 items-end">
                            <div class="lg:col-span-6 flex flex-col gap-1">
                                ...
                                <AppSelect ref="detailAccountRef" v-model="formDetail.rekening" :options="rekeningOptions" value-prop="kd_rek" label="display" placeholder="Cari kode / nama rekening..." />
                            </div>
                            <div class="lg:col-span-2 flex flex-col gap-1">
                                <label class="text-xs font-semibold px-1" :class="{ 'text-base-content/40': formDetail.kredit > 0 }">Debet (Rp)</label>
                                <input type="number" ref="detailDebetRef" v-model.number="formDetail.debet" :disabled="formDetail.kredit > 0" class="input input-bordered input-sm text-right w-full disabled:bg-base-200/50" min="0" @input="formDetail.kredit = 0" @focus="$event.target.select()" @keydown="handleEnter" />
                            </div>
                            <div class="lg:col-span-2 flex flex-col gap-1">
                                <label class="text-xs font-semibold px-1" :class="{ 'text-base-content/40': formDetail.debet > 0 }">Kredit (Rp)</label>
                                <input type="number" ref="detailKreditRef" v-model.number="formDetail.kredit" :disabled="formDetail.debet > 0" class="input input-bordered input-sm text-right w-full disabled:bg-base-200/50" min="0" @input="formDetail.debet = 0" @focus="$event.target.select()" @keydown="handleEnter" />
                            </div>
                            <div class="lg:col-span-2 flex flex-col gap-1">
                                <button class="btn btn-primary btn-sm w-full gap-1" @click="addDetail" :disabled="!formDetail.rekening || (formDetail.debet <= 0 && formDetail.kredit <= 0)"><Plus class="size-4" /> Tambah</button>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-sm w-full">
                            ...
                            <tbody>
                                ...
                                <tr v-if="form.details.length === 0">
                                    <td colspan="5" class="text-center py-4 text-base-content/50">Belum ada rekening ditambahkan.</td>
                                </tr>
                            </tbody>
                                ...
                                <tr>
                                    <td colspan="2" class="text-right">
                                        <div class="flex items-center justify-end gap-3">
                                            <span v-if="!formBalance && form.details.length > 0" class="badge badge-error badge-sm">Belum Balance (Selisih: {{ formatRupiah(formSelisih) }})</span>
                                            <span v-else-if="formBalance && form.details.length >= 2" class="badge badge-success badge-sm text-white">Balance</span>
                                            TOTAL
                                        </div>
                                    </td>
                                    <td class="text-right" :class="{'text-error': !formBalance}">{{ formatRupiah(formTotalDebet) }}</td>
                                    <td class="text-right" :class="{'text-error': !formBalance}">{{ formatRupiah(formTotalKredit) }}</td>
                                    <td></td>
                                </tr>
                            </tfoot>
                        </table>
                ...
                <div class="bg-base-100 px-6 py-4 border-t border-base-200 flex justify-end gap-2 shrink-0">
                    ...
                    <button class="btn btn-primary btn-sm gap-2 min-w-32" @click="save" :disabled="saving || !formBalance || form.details.length === 0">
