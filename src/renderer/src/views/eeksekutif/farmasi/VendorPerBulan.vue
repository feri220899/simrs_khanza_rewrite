...
async function muatData() {
    loading.value = true
    try {
        data.value = await window.api.eeksekutif.penerimaanVendorPerBulan(tahun.value)
    } catch (err) {
...
<template>
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div><h1 class="text-xl font-semibold">Nilai Penerimaan Vendor Farmasi Per Bulan</h1></div>
            <form @submit.prevent="muatData" class="flex flex-wrap items-end gap-3 bg-base-100 p-3 rounded-xl border border-base-200 shadow-sm">
                <label class="form-control">
                    <span class="label py-0.5"><span class="label-text text-xs">Tahun</span></span>
                    <span class="input input-sm input-bordered flex items-center gap-2"><Calendar class="size-4 text-base-content/50" /><input v-model="tahun" type="number" class="w-24" required /></span>
                </label>
        ...
        <template v-else-if="data">
            ...
            <EEksekutifPie title="Vendor Terbesar" :data="data.chartVendor" />
            <EEksekutifPie title="Tren Penerimaan per Bulan" :data="data.chartBulan" />
        </template>
    </div>
</template>