...
async function muatData() { loading.value = true; try { data.value = await window.api.eeksekutif.penerimaanVendorDapurPerBulan(tahun.value) } catch (err) { showToast(err?.message || 'Gagal memuat penerimaan vendor dapur', 'error') } finally { loading.value = false } }
const filteredItems = computed(() => !data.value ? [] : data.value.items.filter(item => item.nama_suplier.toLowerCase().includes(search.value.toLowerCase())))
