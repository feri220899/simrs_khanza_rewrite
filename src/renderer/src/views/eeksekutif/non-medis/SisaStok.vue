...
async function muatData() {
    loading.value = true
    try {
        data.value = await window.api.eeksekutif.sisaStokNonMedis()
    } catch (err) {
        showToast(err?.message || 'Gagal memuat sisa stok non medis', 'error')
    } finally {
        loading.value = false
    }
}
...
const filteredItems = computed(() => {
    ...
    return data.value.items.filter(item => [item.kode_brng, item.nama_brng, item.nm_jenis].some(v => String(v || '').toLowerCase().includes(q)))
})
...
<template>
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div><h1 class="text-xl font-semibold">Sisa Stok & Nilai Aset Non Medis</h1><p class="text-sm text-base-content/60">Laporan inventori barang IPSRS/Non Medis</p></div>
