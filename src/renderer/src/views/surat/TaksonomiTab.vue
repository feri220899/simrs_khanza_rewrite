...
// Komponen GENERIK dipakai buat 9 taksonomi Surat (Rak/Almari/Klasifikasi/
// Sifat/Map/Indeks/Ruang/Status/Balas) — semuanya CRUD identik di kode Java
// asli (lihat SOP di Khanza.md & komentar di SuratTaksonomiService.js), cuma
// beda nama tabel/kolom/prefix. Di-render ulang per jenis lewat `:key` di
// Surat.vue (bukan reaktif terhadap perubahan prop jenis di instance yang sama).
...
const { table, loading, search, fetchData } = useServerTable({
    columns,
    fetchFn: params => window.api.surat.list(props.jenis, params),
    pageSize: 10,
    defaultSortBy: 'kd',
})
...
async function siapkanFormBaru() {
    form.kd   = await window.api.surat.nextKode(props.jenis) // saran kode, tetap bisa diubah manual
    form.nama = ''
}
...
async function simpan() {
    saving.value = true
    try {
        const res = await window.api.surat.create(authStore.token, props.jenis, { ...form })
...
async function update() {
    updating.value = true
    try {
        const res = await window.api.surat.update(authStore.token, props.jenis, editOldKode.value, { ...editForm })
        if (!res.success) { showToast(res.message, 'error'); return }
...
async function hapus(row) {
    ...
    const res = await window.api.surat.delete(authStore.token, props.jenis, row.kd)
    if (!res.success) { showToast(res.message, 'error'); return }
